...
**"Lightweight Swin Transformer with Prosodic Tokens and Huber-Based HuBERT Distillation for Cross-Lingual Speech Emotion Recognition"** *(International Journal of Intelligent Engineering & Systems - IJIES)*.
...
### Key Contributions:
- **Dual-Branch Input:** Fuses high-resolution log-Mel spectrograms with lightweight explicit prosodic tokens (pitch, energy, ZCR, rate).
- **Progressive Receptive Field:** Stage-wise local-window attention schedule (3×3 → 5×5 → 7×7 → 7×7) to aggregate local transients and long-range prosody dynamically.
- **Huber-Based Knowledge Distillation:** Uses a frozen HuBERT teacher (16kHz raw waveform input) to distill acoustic priors into the student (2D spectrogram input) via a robust Huber alignment loss.
...
## 📊 Results Summary
Our compact student (0.85M parameters) outperforms massive self-supervised models and prior hybrid networks:
